...
image_array = np.zeros((1, height * width * channel))			# the image size is four dimension 
label_array = np.zeros((1, 4), 'float')							# the label size is (1,4)
training_data = glob.glob('dataset/*.npz')						# module 's path
# i = 1

# find whether the data zip exit
if not training_data:
    print("No training data in directory, exit")
    sys.exit()

# unzip the data we collect 
for single_npz in training_data:
    with np.load(single_npz) as data:
        train_temp = data['train']								# get the data 
        train_labels_temp = data['train_labels']				# get the label 
    image_array = np.vstack((image_array, train_temp))			# put the data 
    label_array = np.vstack((label_array, train_labels_temp))	# put the label
    # i += 1
...
# split the train data and test data ; proportion = 3 ：7
...
def generatebatch(X, Y, n_examples, batch_size):
    for batch_i in range(n_examples // batch_size):
        ...
        batch_xs = X[start:end]								# get one batch data 
        batch_ys = Y[start:end]								# get one batch data 
...
with tf.Session() as sess:
    ...
    for epoch in range(1000): # 迭代1000个周期
        print("*******************************************************************")
        for batch_xs,batch_ys in generatebatch(X,Y,Y.shape[0],batch_size): 	# get one batch data 
            sess.run(train_step,feed_dict={tf_X:batch_xs,tf_Y:batch_ys})	# feed data to session 
        res = sess.run(accuracy,feed_dict={tf_X:X,tf_Y:Y})					# get the accuracy 
